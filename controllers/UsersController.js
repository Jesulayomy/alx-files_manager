...
export default class UsersController {
  static async postNew (request, response) {
    ...
    const result = await (await dbClient.usersCollection()).insertOne({ email, password: shaOnePassword });
    userQueue.add({ userId: result.insertedId.toString() });
    return response.status(201).send({ id: result.insertedId.toString(), email: email });
  }
